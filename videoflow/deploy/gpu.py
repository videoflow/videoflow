...
def _remove_mig_config_entries(live_config : str, mine : List[str]) -> tuple[str, List[str]]:
    ...
    excluded — it is shared plumbing, not a flow's geometry). An empty
    ``remaining`` is the last-one-out signal: cleanup may then restore the
    ClusterPolicy pointer and delete the map.
    '''
    ...
    remaining = sorted(name for name in configs
                       if name.startswith('videoflow-') and name != MIG_DISABLED_CONFIG)
    stripped = dict(live)
...
class MixGpu(ExclusiveGpu):
    ...
    def prepare(self, demand : Optional[dict[str, int]] = None, kubectl : str = 'kubectl',
        ...
        _publish_mig_configmap(kubectl, namespace,
                               lambda live: _merge_mig_configs(base, config, live_config = live))
    ...
    def cleanup(self, kubectl : str = 'kubectl', flow_id : Optional[str] = None) -> None:
        ...
        A node that failed keeps its annotation, and the policy patch and
        ConfigMap stay wired, so a retried teardown can resume.
        ...
        orphans : List[str] = []           # claimed nodes that never got geometry
        unrestored = False
        for node in nodes:
            ...
            if MIG_RESTORE_ANNOTATION not in annotations:
                ...
                # entry may exist (publish precedes labeling), so it still counts
                # for entry removal below.
                orphans.append(name)
                try:
        ...
        # nodes still resolve their videoflow-<node> labels in the mounted file.
        mine = [f'videoflow-{name}' for name in sorted(set(restored) | set(orphans))]
        strip_namespace = next((ns for ns, _pod in self._mig_manager_pods(kubectl)), None)
        if strip_namespace is not None:
            live = _operator_configmap_yaml(kubectl, strip_namespace, MIG_CONFIGMAP_NAME)
