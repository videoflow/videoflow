...
def _host_port(url, default_port) -> tuple:
    parsed = urlparse(url)
...
def nats_running(url = DEFAULT_NATS_URL) -> bool:
    return port_open(*_host_port(url, 4222))

def redis_running(url = DEFAULT_REDIS_URL) -> bool:
...
def _start(component) -> None:
    _remove_container(CONTAINERS[component])
    _run_docker(_docker_run_argv(component), f'start dev {component}')

def ensure_local_infra(need_redis = True, nats_url = None, redis_url = None) -> Tuple[dict, List[str]]:
    '''
...
def wait_local_infra_ready(created, urls, timeout_secs = 45) -> None:
    '''
...
def teardown_local_infra(components) -> None:
