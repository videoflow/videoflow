...
> `dlq replay` still does not acquire the target's reader obligations. Phase 4 added the allocation
> backends (`deploy/allocation_local.py`, `allocation_kubernetes.py`, the render-only
> `allocation_dra.py` with its `DraEnvironment` matrix and `--gpu-mode dra`), `--gpu-policy
> strict|shared` with the declared-peak budget and `ENV-14` (`VF_GPU_GRANT_JSON`, emitted under
> `strict` or the switch), UUID/MIG-UUID masks resolved with the CUDA runtime's own rules,
> the worker-side grant check (`gpu_fallback`, `requires_peer_access`) and asset identity check
> (`required_assets()`), the explicit `AllocationPlan`, the occupancy re-read before any geometry
> write, readiness correlated with the operation (state **and** advertised slices; the restore
> direction by an observed transition), cleanup that keeps a held node's geometry and the node
> records until the entries are stripped, key-presence restore records (`MIG_LABEL_ABSENT`),
> static MIG consumption and DRA-owned node exclusion, `--gpu-nodes`, `--rollout-policy` with its
> spare-capacity admission, `--resources` with descriptor host requests, and the
> desired/admitted/ready split (`replica_admission`).
